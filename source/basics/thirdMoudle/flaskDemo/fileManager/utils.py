...
from source.basics.myTool.filemanager.excel_manage import save_info
...
class FileManage:
    ...
    def get_current_dir_filename(self, dir_path=None, detail=True):
        """获取当前目录下的文件"""
        ...
        if detail:
            ret = [self.get_file_params('..')]

            for i in range(len(file_list)):
                file_name = os.path.join(dir_path, file_list[i])
                ret.append(self.get_file_params(file_name))
            sorted(ret, key=lambda x: x['type'], reverse=True)
        else:
            ret = ['..']
            for ff in file_list:
                ret.append(ff)
