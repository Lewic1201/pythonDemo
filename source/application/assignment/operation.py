...
class Operation:
    ...
    def register(self):
        ...
        print("恭喜%s开卡成功,您的卡号为:%s,卡的额度为%.2f元!" % (name, cardid, card.limit_money))
        ...
        if not card:
            ...
            if card.islock:
                ...
                if self.check_pwd(card):
                    print("卡内余额%.2f元" % (card.get_balance(int(time.time()))))
            ...
            if card.islock:
                ...
                if sure == "1":
                    ...
                    if money < 0:
                        ...
                        # card.money += money
                        card.set_change_money(money, now_time, 'r')
                        print("成功存入%.2f元。" % (money))
                        print("当前余额为%.2f元" % (card.get_balance(now_time)))
                elif sure == "0":
                    return
        ...
        if not card:
            ...
            if card.islock:
                ...
                if self.check_pwd(card):
                    ...
                    if card.money - money >= -10000:
                        # card.money -= money
                        # 如果小于50s还钱,锁卡
                        last_time = card.get_last_timestamp()
                        if (now_time - last_time) <= 50 or last_time == 0:
                            card.set_change_money(money, now_time, 'b')
                        else:
                            self.lock()
                        print("成功取走%.2f元,卡内额度为%.2f元,卡内余额为%.2f元。" % (money, card.limit_money, card.money))
                    else:
                        # print("输入金额有误,请重新输入!")
                        print("贷款金额应小于10000元!")
                    ...
                    if otherid not in self.user_dict:
                        ...
                        if sure == "1":
                            ...
                            if money > 0 and money <= card.money:
                                ...
                                print("您向%s成功转账%.2f元" % (other_user.name, money))
                            else:
                                print("转账金额有误!")
                        else:
                            return
